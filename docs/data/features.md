...
This preserves same-session candle geometry while preventing stock splits and dividend adjustments from appearing as overnight gaps, directional movement, volatility, momentum, or market-structure changes. Source volume is not transformed. Model-facing features such as rolling VWAP and MFI therefore combine OHLC values expressed on the `adjusted_close` scale with the provider's historical volume series. These are corporate-action-consistent research features, not literal reconstructions of historical traded-price VWAP or MFI. The implementation assumes that the provider's volume history is compatible with its adjusted price history around stock splits. Turnover instead multiplies provider `close` by provider `volume`, which is already split-invariant because price and share count move inversely.
...
- every parameter value rather than relying on function defaults;
...
assert tuple(features.columns[-len(DEFAULT_FEATURE_SET.feature_columns) :]) == (
    DEFAULT_FEATURE_SET.feature_columns
)

manifest = DEFAULT_FEATURE_SET.to_manifest()
```

A smaller, independently identified set can select complete family blocks without calculating the other families:

```python
baseline_set = DEFAULT_FEATURE_SET.select(
    "returns",
    "trend",
    "volume",
    name="baseline_ohlcv",
...
baseline_features = add_feature_set(prices, feature_set=baseline_set)
```
