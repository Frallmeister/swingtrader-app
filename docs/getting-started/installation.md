...
The optional `modeling` extra installs scikit-learn and matplotlib for baseline modeling
and evaluation reporting, together with the lightweight MLflow client and the SQL storage
dependencies used by local experiment tracking. Install it by itself when you only need the
modeling workflow:
...
The full MLflow server and UI are intentionally not part of the project environment.
See [Experiment Specifications and MLflow Tracking](../modeling/experiments.md) for
the local SQLite workflow and the optional command for launching the UI.
