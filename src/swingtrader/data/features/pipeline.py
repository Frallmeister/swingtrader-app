...
def add_default_features(data: pd.DataFrame) -> pd.DataFrame:
    ...
    The families run in an explicit, stable order: returns, trend, momentum, then
    volatility. The input must satisfy the canonical market-price contract and
    provide every column the individual families require, including ``high``,
    ``low``, ``close``, ``adjusted_close``, and ``volume``. The input index and
